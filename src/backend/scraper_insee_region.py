...
def _read_tables(url: str):
    logger.info("Téléchargement et parsing des tableaux HTML depuis %s", url)
...
def fetch_population() -> pd.DataFrame:
    logger.info("Extraction population régionale depuis %s", URL_POP)
...
def fetch_rev_pov(pop: pd.DataFrame) -> pd.DataFrame:
    logger.info("Extraction revenu médian et pauvreté depuis %s", URL_REVPOV)
    tbl = next(
        t
        for t in _read_tables(URL_REVPOV)
...
if __name__ == "__main__":
    ...
    # contrôle rapide
    logger.info("🗹 Populations récupérées : %d", len(pop))
    logger.info("🗹 Lignes finales         : %d", len(df))

    DB.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(DB) as conn:
        df.to_sql("region_indicators", conn, if_exists="replace", index=False)

    logger.info("✅ region_indicators mis à jour : %d régions (DB: %s)", len(df), DB)
