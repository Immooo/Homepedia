...
SELECT
    *,
    CAST(
        REPLACE(REPLACE(valeur_fonciere, ' ', ''), ',', '.') 
    AS REAL) AS valeur_fonciere_num,
    surface_reelle_bati AS surface_bati_num
FROM transactions
"""
...
df = df[df['surface_bati_num'] > 0]
df['prix_m2'] = df['valeur_fonciere_num'] / df['surface_bati_num']
...
print(df[['valeur_fonciere_num','surface_bati_num','nombre_pieces_principales']].describe())

# 5. Transactions par département
